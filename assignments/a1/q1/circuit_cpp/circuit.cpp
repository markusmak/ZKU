...
void merkle_root_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void merkle_root_3_run(uint ctx_index,Circom_CalcWit* ctx);
void merkle_tree_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void merkle_tree_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 19509;}
...
ctx->componentMemory[coffset].templateName = "merkle_root";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "memory_components";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
ctx->componentMemory[coffset].templateName = "merkle_tree";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 8; i++) {
std::string new_cmp_name = "memory_components"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
uint aux_positions [7]= {2,4,6,8,10,12,14};
...
mySubcomponents[aux_create+i] = aux_cmp_num;
merkle_root_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[17]); // line circom 26
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[16];
// load src
Fr_add(&expaux[0],&lvar[16],&circuitConstants[3]); // line circom 26
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[17]); // line circom 26
...
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[31]); // line circom 34
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[16])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_sub(&expaux[0],&lvar[16],&circuitConstants[3]); // line circom 36
// end load src
Fr_copy(aux_dest,&lvar[((1 * Fr_toInt(&expaux[0])) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
merkle_root_3_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[16],&circuitConstants[5]); // line circom 37
// end load src
Fr_copy(aux_dest,&lvar[((1 * Fr_toInt(&expaux[0])) + 1)]);
}
// run sub component if needed
...
Fr_div(&expaux[2],&lvar[16],&circuitConstants[5]); // line circom 38
Fr_add(&expaux[1],&expaux[2],&circuitConstants[17]); // line circom 38
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 38
{
PFrElement aux_dest = &lvar[((1 * Fr_toInt(&expaux[0])) + 1)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[16])) + 8)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[16];
// load src
Fr_add(&expaux[0],&lvar[16],&circuitConstants[5]); // line circom 34
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[31]); // line circom 34
...
merkle_tree_4_create(1,0,ctx,"main",0);
merkle_tree_4_run(0,ctx);
}
